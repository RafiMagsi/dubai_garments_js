...
import { PipelineInsightRequestSchema } from '@/lib/ai-sales-agent/contracts';
import { runPipelineInsights } from '@/lib/ai-sales-agent/pipeline-insights';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const parsed = PipelineInsightRequestSchema.safeParse(body);

    if (!parsed.success) {
      return NextResponse.json(
        {
          ok: false,
          message: getAiPayloadValidationMessage(parsed.error, 'Invalid pipeline insight payload.'),
    ...
    let result;
    try {
      result = await runPipelineInsights({
        ...parsed.data,
        context: {
          userId: session.sub,
          role: session.role,
    ...
    const message =
      error instanceof Error ? error.message : 'Failed to run pipeline insights.';
