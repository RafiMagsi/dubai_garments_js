  ...
  return (
    <div>
      ...
      <div className="pins-grid">
        ...
        <div>
          <AisFieldLabel>Request Timeout (ms)</AisFieldLabel>
          <TextField
            className="pins-input"
            type="number"
            ...
            disabled={loading || saving}
            data-testid="model-settings-request-timeout-ms-input"
          />
        </div>

        <div>
          <AisFieldLabel>Max Retries</AisFieldLabel>
          <TextField
            className="pins-input"
            type="number"
            value={String(config.maxRetries)}
            onChange={(event) => onChange({ maxRetries: Number(event.target.value || 0) })}
            disabled={loading || saving}
            data-testid="model-settings-max-retries-input"
          />
        </div>

        <div>
          <AisFieldLabel>Retry Backoff (ms)</AisFieldLabel>
          <TextField
            className="pins-input"
            type="number"
            value={String(config.retryBackoffMs)}
            onChange={(event) =>
              onChange({ retryBackoffMs: Number(event.target.value || 0) })
            }
            disabled={loading || saving}
            data-testid="model-settings-retry-backoff-ms-input"
          />
        </div>
      </div>
